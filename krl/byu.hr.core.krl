...
from #{wrangler:channels().filter(function(c){c.get("id")==subs:inbound().head().get("Rx")}).head().get("tags").filter(function(t){t!="relationship"}).head().split("-").filter(function(n){n!=this_person}).head()}
